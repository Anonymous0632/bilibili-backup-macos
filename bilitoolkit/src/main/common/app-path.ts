 ...
 * macOS 打包后 .app 通常不可作为数据写入目录，生产环境改用 userData。
 */
const programRoot = devUrl ? appRootPath : process.platform === 'darwin' ? app.getPath('userData') : path.dirname(app.getPath('exe'))
/**
 * 构建后的资源路径（开发环境不要使用）
 */

...
export abstract class BaseWindowManager {
  ...
  public async createPluginView(context: ApiCallerContext, plugin: InstalledToolkitPlugin) {
    ...
    try {
      if (isHttpUrl(plugin.files.indexPath)) {
        // 测试环境：开发服务器URL
        await view.webContents.loadURL(plugin.files.indexPath)
      } else {
        const indexPath = path.isAbsolute(plugin.files.indexPath)
          ? plugin.files.indexPath
          : path.resolve(appPath.pluginsPath, plugin.files.indexPath)
        await view.webContents.loadFile(indexPath)
      }
    } catch (error) {
      this.webContentsToPluginMap.delete(webContentsId)
      this.webContentsToViewMap.delete(webContentsId)
      this.webContentsToWindow.delete(webContentsId)
      this.pluginToViewMap.delete(plugin.id)
      this.pluginResizeListeners.delete(plugin.id)
      window.removeListener('resize', updateBoundsListener)
